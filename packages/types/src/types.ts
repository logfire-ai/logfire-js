...
/**
 * Interface representing a minimal Logfire log
 */
export interface ILogfireLog {
  dt: Date;
  level: ILogLevel;
  message: string;
...
/**
 * Middleware type, which takes a log, and returns Promise<ILogfireLog>
 */
export type Middleware = (log: ILogfireLog) => Promise<ILogfireLog>;

/**
 * Sync type, which takes a array of logs, and resolves the logs
 */
export type Sync = (logs: ILogfireLog[]) => Promise<ILogfireLog[]>;
